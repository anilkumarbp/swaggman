...
module.exports = (swaggerJSON, ...options) => {
    ...
    if('object' !== typeof swaggerJSON) {
        ...
        if(!swaggerJSON.info.hasOwnProperty('title') && !process.env.POSTMAN_NAME) throw new Error('Missing property `title` on info object supplied as argument to translator.info()');
    ...
    let postmanName = process.env.POSTMAN_NAME || swaggerJSON.info['title'];
    let postmanSchema = process.env.POSTMAN_SCHEMA || 'https://schema.getpostman.com/collection/v2';
